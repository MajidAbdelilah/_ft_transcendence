...
class PingPongConsumer(AsyncWebsocketConsumer):
    ...
    def update_game_state(self):
        ...
        for player in self.players:
            ...
            elif self.players[player]['direction'] == 'down' and self.players[player]['y'] < 100:
                self.players[player]['y'] += 1

        # Ball collision with player paddles
        if self.ball['x'] <= 0 and self.players['player1']['y'] - 10 <= self.ball['y'] <= self.players['player1']['y'] + 10:
            self.ball['vx'] *= -1
        elif self.ball['x'] >= 100 and self.players['player2']['y'] - 10 <= self.ball['y'] <= self.players['player2']['y'] + 10:
            self.ball['vx'] *= -1
